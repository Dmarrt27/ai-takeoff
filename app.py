...
_API_CALL_CONCURRENCY = int(os.environ.get("API_CALL_CONCURRENCY", "2"))
...
# Default lowered to 160px after a 256px-default deploy OOMed on Render's
# 512MB plan — sheet-type classification doesn't need legible callouts.
TRIAGE_THUMB_PX = int(os.environ.get("TRIAGE_THUMB_PX", "160"))
TRIAGE_GRID_COLS = int(os.environ.get("TRIAGE_GRID_COLS", "3"))
TRIAGE_GRID_ROWS = int(os.environ.get("TRIAGE_GRID_ROWS", "2"))
# Confirm-pass batch size: how many tiles get sent per confirm-pass call.
# Smaller batches → smaller in-flight serialized payloads under concurrent
# fan-out; raise when the host has more RAM than Render's 512MB plan.
CONFIRM_BATCH_TILES = int(os.environ.get("CONFIRM_BATCH_TILES", "20"))
...
def _plan_tiles_for_pages(pdf_path, pages_1based, token_budget, image_budget):
    ...
    try:
        for page_no in pages_1based:
            if used_images >= image_budget or used_tokens >= token_budget:
                break
            i = page_no - 1
            try:
                page = pdf[i]
    ...
    try:
        ...
        try:
            ...
            if len(page_texts) > SMALL_SET_THRESHOLD:
                ...
                if triage:
                    # Triage held thumbnail buffers in RAM — free them
                    # before the high-res tile pass so peak memory stays
                    # bounded on tight hosts (Render's 512MB plan OOM'd
                    # without this).
                    gc.collect()
